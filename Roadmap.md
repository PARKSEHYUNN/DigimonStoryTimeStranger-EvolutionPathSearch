...
3. **데이터 3중 중복** — `evolution.from` / `evolution.to` / `evolution_requirements`가 같은 간선을 세 번 표현한다. 이미 역링크 4건이 어긋나 있다(455↔456, 463→309, 464→423).

   > **정정 (구현 중 확인):** 이 4건은 사용자에게 보이는 버그를 일으키지 않았다. 네 경우 모두 반대편 `.to`에 간선이 이미 선언되어 있고 요구조건도 양쪽 다 존재해서, 그래프 연결성과 조건 표시 모두 정상이었다. 빠진 것은 중복 표현인 `.from` 항목뿐이다. 따라서 정규화가 고치는 것은 *현재 증상*이 아니라 *드리프트가 가능한 구조* 자체다.
...
- 방향 간선 1,118개 중 17개가 상호 쌍(X항체·Alter 형태의 양방향 전환)이라 무방향 기준 1,101개가 된다.
...
사이트 전체가 렌더하는 문자는 806종이다. 원본 폰트는 키릴·라틴확장·옛한글·전체 CJK를 싣고 있었다.

| | 원본 | 서브셋 | |
|---|---|---|---|
| `PretendardVariable` (라틴+한글) | 2,009 KB | **586 KB** | -70.9% |
| `PretendardJPVariable` (가나+한자) | 5,223 KB | **131 KB** | -97.5% |
| 합계 | 7,232 KB | **716 KB** | **-90.1%** |

- **한글은 현대 음절 11,172자를 전부 넣었다.** 사이트가 쓰는 390자만 넣으면 105 KB지만, 검색창·버그신고에 사용자가 입력한 글자가 폴백 폰트로 떨어진다. 481 KB를 더 써서 **커버리지 손실 0**을 샀다. 한국어가 주 사용자층인 사이트에서 타이핑 중 글꼴이 바뀌는 건 받아들일 수 없다.
- **한자는 사이트가 쓰는 207자만 넣었다.** JIS 1수준까지 확장하면 1,470 KB가 된다. 차이가 드러나는 건 일본어 사용자가 *직접 입력한* 한자뿐이고, 그건 어느 기기에서나 시스템 폰트로 정상 표시된다. 본문은 어느 쪽이든 완전히 커버된다.
- 한국어 페이지는 일본어를 **한 글자도** 렌더하지 않는다(실측). JP 폰트는 `/ja/`에서만 요청된다.

**발견한 실제 버그**

`unicode-range`가 CJK 블록 전체(`U+4E00-9FFF`, 20,992자)를 주장하는데 서브셋에는 207자뿐이었다.
이 상태에서 커버되지 않은 한자는 시스템 폰트로 폴백되는 게 아니라 **두부(tofu)로 렌더된다** —
브라우저가 "이 폰트가 처리한다"고 믿고 폴백을 시도하지 않기 때문이다.

손으로 유지할 수 없는 값이므로 `scripts/subset-fonts.mjs`가 `src/app/fonts.css`를 생성하게 했다.
서브셋 파일과 `@font-face` 선언이 같은 곳에서 나오므로 둘이 어긋날 수 없다.
기존 선언은 `—` `…` `©` `·`와 일본어 문장부호도 빠뜨리고 있어 그 글자들이 시스템 폰트로 떨어지고 있었다.

**추가한 검사**

- `check:fonts` — 익스포트된 HTML의 모든 문자가 서브셋에 있는지, `unicode-range`가 서브셋보다 넓거나 좁지 않은지. 정적 검사라 빠르다.
- `check:assets` — 브라우저에서 로케일별 폰트 요청, 캔버스 폭 비교로 폴백 렌더 탐지, 아이콘 응답. 정적으로는 볼 수 없는 것들.

**캐시 정책** (`public/_headers`)

콘텐츠 해시가 붙은 건 `/_next/static/*`뿐이다. 여기에만 `immutable`을 준다.
`/icons/42.webp`와 폰트 파일은 내용이 바뀌어도 이름이 그대로이므로 7일 + SWR 30일로 둔다.
1년 `immutable`을 주면 아이콘을 교체해도 1년간 반영되지 않는다.

**배포 절차는 [DEPLOY.md](DEPLOY.md)에 별도로 정리했다.**
